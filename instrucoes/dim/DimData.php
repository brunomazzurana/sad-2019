...
class DimData{
    public function extrairETransformarDatas(){
        ...
        if($resultDimData->num_rows === 0){//Dimensao está vázia
            $sqlDataPedido = $connComercial->prepare('SELECT data_pedido FROM pedido');
            $sqlDataPedido->execute();
            $resulDataPedido = $sqlDataPedido->get_result();
    ...
    public function carregarNovaData($objData){
        ...
        $sqlInsertData = $connDimensao->prepare('INSERT INTO dim_data(data, dia, mes, ano, semana_ano, bimestre, trimestre, semestre)
                                                 values 
                                                 (?,?,?,?,?,?,?,?)');
        $sqlInsertData->bind_param('ssssssss', $objetoData->data, $objData->dia, $objData->mes, $objData->ano,
